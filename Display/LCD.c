...
extern int controllerWorkMode; // OFF - manual mode, ON - auto mode

extern unsigned char doseSandCmd;
extern unsigned char doseGravelCmd;
extern unsigned char doseCementCmd;
extern unsigned char doseWaterCmd;
extern unsigned char emptyCementCmd;
extern unsigned char emptyWaterCmd;
extern unsigned char openCloseMixerCmd;
extern unsigned char skipCartUpCmd;
extern unsigned char skipCartDownCmd;
...
void InitLCD(void)
{
    ...
    sprintf(Row3, " concrete controller!");
    sprintf(Row4, "       Enjoy!       ");
...
void LCDTask(void)
{
    ...
    unsigned int restMixingTime;
    
    // buffers for valve string commands - 
    char sandDosing;                 // R - run, S - stop
    char gravelDosing;               // R - run, S - stop
    char cementDosing;               // R - run, S - stop
    char waterDosing;                // R - run, S - stop
    char mixerValve;                 // R - change current state(open - close or close - open), S - not active
    char waterValve;                 // O - open, C - close
    char cementValve;                // O - open, C - close
    char cartCmd;                    // U - up, D - down
    ...
    if(controllerWorkMode == OFF)
    {
        ...
        // cart command "Up"
        if(skipCartUpCmd == ON && skipCartDownCmd == OFF)
        {
            cartCmd = 'U';
        }
        else if(skipCartUpCmd == OFF && skipCartDownCmd == ON)
        {
            cartCmd = 'D';
        }
        else if(skipCartUpCmd == OFF && skipCartDownCmd == OFF)
        {
            cartCmd = ' ';
        }
        else // skipCartUpCmd == ON && skipCartDownCmd == ON
        {
            cartCmd = '!';
        }
                
        sprintf(Row1, "Mode:         %6s", "Manual");
        sprintf(Row2, "      Commands      ");
        sprintf(Row3, "SD:%c,GD:%c,CD:%c,WD:%c ", sandDosing, gravelDosing, cementDosing, waterDosing);
        sprintf(Row4, "CV:%c,WV:%c,MV:%c,CCm:%c", cementValve, waterValve, mixerValve, cartCmd);
